...
#set happiness and stress values
# happy = {}
# stressy = {}
# n = 10
# for i in range(n):
...
# D = {1:1, 2:1, 3:1, 4:2, 5:1, 6:2, 7:2, 8:1, 9:2, 10:2}
# G = nx.Graph()
# G.add_nodes_from(range(n))
# #add an edge from i -> j with data h=hij and s = sij
# for i in range(n):
#     for j in range(n):
#         if i < j:
#             G.add_edge(i, j, {happiness: happy[(i,j)]}, {stress: stressy[(i,j)]})
# s = 10
# rooms = 2
# yes = is_valid_solution(D, G, s, rooms)

def calculate_stress(D, G):
    """
    Calculates the total stress in mapping D by summing the stress of each room.
    Args:
        D: Dictionary mapping student to room
        G: networkx.Graph
        s: Stress budget
    ...
    for k, v in room_to_s.items():
        room_stress = calculate_stress_for_room(v, G)
        stress_total += room_stress
    # for k in room_to_s.keys():
    #     v = room_to_s[k]
    #     room_stress = calculate_stress_for_room(v, G)
    #     stress_total += room_stress
    return stress_total
