...
def sorted_k_partitions(seq, k, start):
...
##solver just for medium-60
#def tempe4(G, s):


#solver for only medium-21 (r=6)
#medium-37 input (r=6)
#medium-127 input (r=4)
#medium-83 (r=6)
#medium-118 (r=7)
#medium-124
#medium-189
...
def tempe3(G, s):
    ...
    for room in range(1, 50):
        #print(room, ": ")
        _, maps, num = dp(G, s, room)
        happiness = calculate_happiness(maps, G)
        if is_valid_solution(maps, G, s, num):
            print(happiness)
            allz[happiness] = (maps, num)
    best = max(allz.keys(), key=lambda x:x)
    return allz[best][0], allz[best][1]
...
def solve_greedy(G, s):
    ...
    for pair in edgeview:
        ...
        #ratio = happy/stress
        if happy == 0:
            happy = 0.000000000000000000001
        # if stress == 0:
        #     stress = -0.0000000000000000000001
        ratio = (happy, stress)
